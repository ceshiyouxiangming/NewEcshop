...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        if (!empty($_FILES['img_file_src']['name']))
        {
            ...
            //$name .= '.' . end(explode('.', $_FILES['img_file_src']['name'])); 20170220 edit by JimmyChoi
            $files_explode = explode('.', $_FILES['img_file_src']['name']);
            $name .= '.' . end($files_explode);
        ...
        //set_flash_data($_CFG['flash_theme'], $error_msg = ''); 20170220 edit by JimmyChoi
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        if (!empty($_FILES['img_file_src']['name']))
        {
            ...
            //update to php5.6 by JimmyChoi
            $explode_files = explode('.', $_FILES['img_file_src']['name']);
            $name .= '.' . end($explode_files);
...
elseif ($_REQUEST['act'] == 'custom_insert')
{
    ...
    if ($ad_img['ad_img']['name'] && $ad_img['ad_img']['size'] > 0)
    {
        ...
        //update to php5.6 By JimmyChoi
        $explode_ad_img = explode('.', $ad_img['ad_img']['name']);
        $name .= '.' . end($explode_ad_img);
...
function get_url_image($url)
{
    ...
    $ext = strtolower(end($explode_url));
