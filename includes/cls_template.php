...
class cls_template
{
    ...
    function fetch_str($source)
    {
        ...
        if(preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $source, $sp_match))
        {
         ...
         return preg_replace_callback("/{([^\}\{\n]*)}/", function($r) { return $this->select($r[1]); }, $source);//upgrade for 5.6
    }
...
//            }
        ...
        {
            $tag_sel = explode(' ', $tag);//upgrade for 5.6
			$tag_sel = array_shift($tag_sel);//upgrade for 5.6
            switch ($tag_sel)
            {
                case 'if':
    ...
    function get_val($val)
    {
        ...
        {
            $val = preg_replace_callback("/\[([^\[\]]*)\]/is", function($r) { return '.'.str_replace('$','$',$r[1]);}, $val);//upgrade for 5.6
        }
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            $source      = preg_replace_callback('/<!--\s#BeginLibraryItem\s\"\/(.*?)\"\s-->.*?<!--\s#EndLibraryItem\s-->/s',function($r) { return '{include file='.strtolower($r[1]). '}';}, $source);//upgrade for 5.6
